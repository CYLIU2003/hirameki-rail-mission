(function(global){
  ...
  // ※effectは「良いほど増える」スコア（救援/混雑/遅延）
  // トレードオフは、別スコアを下げたり friction で表現します
  const actions = [
    { id:"none", label:"（何もしない）", effect:{rescue:0,crowd:0,delay:0}, description:"作戦なし" },

    { id:"prioritize_rescue", label:"救援を最優先にする", effect:{rescue:+9,crowd:0,delay:-2},
      description:"救援が速くなるが、全体遅延が少し悪化しがち" },

    { id:"add_local", label:"各停を増発する", effect:{rescue:0,crowd:+9,delay:-3},
      description:"混雑が改善するが、遅延が少し積み上がる" },

    { id:"cancel_express", label:"特急を見合わせる", effect:{rescue:-1,crowd:+7,delay:+7},
      description:"運用が単純化して遅延改善、混雑も下がる（救援は少し後回し）" },

    { id:"shorten_turnback", label:"折返しを短縮する", effect:{rescue:0,crowd:-1,delay:+9},
      description:"遅延が大きく改善する（混雑は少し悪化しがち）" },

    { id:"platform_change", label:"番線を変更する", effect:{rescue:+2,crowd:+8,delay:+2},
      description:"導線を作って混雑を解消、遅延も少し改善" },

    { id:"detour", label:"迂回運転を入れる", effect:{rescue:0,crowd:-2,delay:+7},
      description:"遅延が改善するが、人の流れが少し乱れる" },

    { id:"info_guide", label:"案内を強化する", effect:{rescue:0,crowd:+7,delay:+3},
      description:"万能。大崩れしにくい" },

    // 追加で“中身”を増やした対策（遊びの幅）
    { id:"hold_doors", label:"乗降を分散（ドア案内）", effect:{rescue:0,crowd:+6,delay:+1},
      description:"混雑をじわっと改善、遅延にも少し効く" },

    { id:"skip_stops", label:"一部通過（臨時快速化）", effect:{rescue:+1,crowd:-3,delay:+8},
      description:"遅延が改善するが、混雑は悪化しやすい" },

    { id:"split_flow", label:"乗換分散（行先分割）", effect:{rescue:0,crowd:+9,delay:0},
      description:"混雑特化の強手（遅延は動かない）" },

    { id:"dispatch_extra_staff", label:"駅員増員（誘導強化）", effect:{rescue:0,crowd:+6,delay:+4},
      description:"案内強化の上位互換寄り（混雑と遅延に効く）" },
  ];

  // カードの設計方針：
  // - initScores: 初期の“良さ”
  // - baseStep: 何もしなくても時間でどう動くか（プラスは改善）
  // - friction: 毎ステップの“現実の抵抗”（マイナスされる）
  // - pass: クリア条件
  // - bannedActions: 縛り（選択しても無効化）
  // - onlyOneRule: ルール1個だけ縛り
  const cards = [
    // ===== EASY 01-10 =====
    ...
    { id:"04", difficulty:"EASY",
      ...
      tags:["event","crowd"],
      hint:"「増発」＋「誘導」が鉄板。",
      recommended:["add_local","dispatch_extra_staff"],
    ...
    { id:"10", difficulty:"EASY",
      ...
      tags:["training"],
      hint:"万能の「案内強化」や「駅員増員」が安定。",
      recommended:["info_guide","dispatch_extra_staff"],
    ...
    { id:"14", difficulty:"NORMAL",
      ...
      tags:["crowd","event"],
      hint:"特急見合わせ or 乗換分散。",
      recommended:["cancel_express","split_flow"],
    ...
    { id:"17", difficulty:"NORMAL",
      ...
      tags:["crowd"],
      hint:"案内強化 or 駅員増員。",
      recommended:["dispatch_extra_staff","info_guide"],
    ...
    { id:"18", difficulty:"NORMAL",
      ...
      objective:"遅延75以上。",
      tags:["delay"],
      hint:"折返し短縮が刺さる。快速化も手。",
      recommended:["shorten_turnback","skip_stops"],
    ...
    { id:"20", difficulty:"NORMAL",
      ...
      tags:["event","crowd"],
      hint:"増発 or 分散。",
      recommended:["add_local","split_flow"],
    ...
    { id:"21", difficulty:"NORMAL",
      ...
      tags:["delay","crowd"],
      hint:"迂回＋誘導強化。",
      recommended:["detour","dispatch_extra_staff"],
    ...
    { id:"23", difficulty:"NORMAL",
      ...
      tags:["crowd"],
      hint:"番線変更＋分散が強い。",
      recommended:["platform_change","split_flow"],
    ...
    { id:"25", difficulty:"NORMAL",
      ...
      hint:"万能系＋混雑特化or遅延特化を足す。",
      recommended:["dispatch_extra_staff","shorten_turnback"],
    ...
    { id:"30", difficulty:"HARD",
      ...
      constraints:["作戦は1個だけ（作戦②は無効）"],
      onlyOneRule:true,
      hint:"万能か救援特化、どちらかで勝負。",
      recommended:["dispatch_extra_staff","prioritize_rescue"],
